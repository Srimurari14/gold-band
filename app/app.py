...
if exp is not None:
    ...
    with c1:
        ...
        st.markdown(
            ...
            f"against what actually happens the next trading day, in gold's dollar "
            f"price. Using the price from {exp['data_through']}, it is guessing what "
            f"gold will cost on the next trading day after that:</div>"
            f"<div style='display:flex;gap:2rem;align-items:baseline'>"
...
if fx_exp is not None:
    ...
    with c1:
        st.markdown(
            ...
            f"prices shown above. Using the rate from {fx_exp['data_through']}, "
            f"here is its guess for the next trading day after that:</div>"
            f"<div style='display:flex;gap:2rem;align-items:baseline'>"
